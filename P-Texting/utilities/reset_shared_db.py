#!/usr/bin/env python3
"""
Import CSV and reset failed records to pending in progress_shared.db
"""
import sqlite3
import pandas as pd
import hashlib
...
def reset_shared_db():
    """Import records and reset failed to pending in shared DB"""
    ...
    for idx, row in df.iterrows():
        ...
        sent_by_account = str(row['Sent_By_Account']) if pd.notna(row['Sent_By_Account']) else None
        last_attempted_by_account = None
        
        cursor.execute(
            '''INSERT OR REPLACE INTO messages 
               (phone, name, status, attempts, last_error, last_attempt_at, sent_at, sent_by_account, last_attempted_by_account, message_hash)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (phone, name, status, attempts, last_error, last_attempt_at, sent_at, sent_by_account, last_attempted_by_account, message_hash)
        )
        imported += 1
