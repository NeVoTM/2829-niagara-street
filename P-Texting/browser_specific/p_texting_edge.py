#!/usr/bin/env python3
"""
P-TEXTING - EDGE VERSION
Standalone Edge-only version with dedicated GUI

This is a completely separate program from the Chrome version.
Use this for account2 texting.
"""
...
class PTextingEdge:
    def __init__(self, root):
        self.root = root
        self.root.title("P-Texting - EDGE VERSION")
        ...
        # Header
        header = ttk.Label(main_frame, text="P-TEXTING - EDGE VERSION", 
                          font=('Arial', 16, 'bold'), foreground='blue')
        header.grid(row=0, column=0, columnspan=3, pady=10)
        
        info_label = ttk.Label(main_frame, text="✅ Edge keeps you logged in - Best experience!", 
                              foreground='green')
        info_label.grid(row=1, column=0, columnspan=3, pady=5)
        
        row = 2
        
        # Note about Edge profile - not needed for remote debugging
        note_label = ttk.Label(main_frame, text="ℹ️ Edge profile auto-detected from running Edge instance", 
        ...
        # Account Label (hardcoded for Edge)
        ...
        ttk.Label(settings_frame, text="(Auto: Edge = account2)", foreground='gray', font=('Arial', 8)).grid(row=1, column=2, sticky=tk.W, padx=5)
        ...
        ttk.Button(button_frame, text="🚀 Start Edge Debug", command=self.start_edge_debug, width=20).grid(row=0, column=2, padx=5)
        ...
        # Status
        self.status_var = tk.StringVar(value="Ready - Edge Version")
    ...
    def save_config(self):
        config = {
            "browser": "edge",
            "browser_profile_path": "C:\\temp\\edge_debug_profile",  # Auto from start_edge_debug.bat
            "browser_binary_path": "",
    ...
    def load_config(self):
        ...
        try:
            ...
            # Profile path auto-detected from running Edge, no need to show/load
    ...
    def start_edge_debug(self):
        ...
        bat_file = Path("start_edge_debug.bat")
        if not bat_file.exists():
            messagebox.showerror("Error", "start_edge_debug.bat not found!")
            return
        ...
        try:
            ...
            self.status_var.set("🚀 Edge debug mode started - Check the new window")
            messagebox.showinfo("Edge Started", 
                              "Edge has been started with remote debugging.\n\n"
                              "1. Log in to Google Voice if needed\n"
                              "2. Come back here and click 'SEND MESSAGES'")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to start Edge: {e}")
    ...
    def send_messages(self):
        ...
        try:
            self.status_var.set("📤 Sending messages via Edge...")
            # Run in new console window so user can see progress
            subprocess.Popen([sys.executable, str(script.absolute()), self.config_file],
                           creationflags=subprocess.CREATE_NEW_CONSOLE)
            messagebox.showinfo("Started", 
                              "Message sending started!\n\n"
                              "Check the new console window for progress.\n"
                              "Edge will stay open when done.")
        except Exception as e:
    ...
    def open_report(self):
        ...
        try:
            subprocess.Popen([sys.executable, "generate_report.py", "config_edge.json"],
                           creationflags=subprocess.CREATE_NEW_CONSOLE)
