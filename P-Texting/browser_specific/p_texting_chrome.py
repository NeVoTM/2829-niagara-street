#!/usr/bin/env python3
"""
P-TEXTING - CHROME VERSION
Standalone Chrome-only version with dedicated GUI

This is a completely separate program from the Firefox version.
Use this as your PRIMARY texting tool.
"""
...
class PTextingChrome:
    def __init__(self, root):
        self.root = root
        self.root.title("P-Texting - CHROME VERSION")
        self.root.geometry("800x900")
        
        # Config file
        self.config_file = "config_chrome.json"
        ...
        # Header
        header = ttk.Label(main_frame, text="P-TEXTING - CHROME VERSION", 
                          font=('Arial', 16, 'bold'), foreground='blue')
        header.grid(row=0, column=0, columnspan=3, pady=10)
        
        info_label = ttk.Label(main_frame, text="✅ Chrome keeps you logged in - Best experience!", 
                              foreground='green')
        info_label.grid(row=1, column=0, columnspan=3, pady=5)
        
        row = 2
        
        # Note about Chrome profile - not needed for remote debugging
        note_label = ttk.Label(main_frame, text="ℹ️ Chrome profile auto-detected from running Chrome instance", 
        ...
        # Account Label (hardcoded for Chrome)
        ttk.Label(settings_frame, text="Account Label:").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.account_label_var = tk.StringVar(value="account1")
        ...
        ttk.Label(settings_frame, text="(Auto: Chrome = account1)", foreground='gray', font=('Arial', 8)).grid(row=1, column=2, sticky=tk.W, padx=5)
        ...
        ttk.Button(button_frame, text="🚀 Start Chrome Debug", command=self.start_chrome_debug, width=20).grid(row=0, column=2, padx=5)
        ...
        # Status
        self.status_var = tk.StringVar(value="Ready - Chrome Version")
    ...
    def save_config(self):
        config = {
            "browser": "chrome",
            "browser_profile_path": "C:\\temp\\chrome_debug_profile",  # Auto from start_chrome_debug.bat
            "browser_binary_path": "",
    ...
    def load_config(self):
        ...
        try:
            ...
            # Profile path auto-detected from running Chrome, no need to show/load
            ...
            self.account_label_var.set(config.get('account_label', 'account1'))
    ...
    def start_chrome_debug(self):
        ...
        if not bat_file.exists():
            messagebox.showerror("Error", "start_chrome_debug.bat not found!")
            return
        ...
        try:
            ...
            self.status_var.set("🚀 Chrome debug mode started - Check the new window")
            messagebox.showinfo("Chrome Started", 
                              "Chrome has been started with remote debugging.\n\n"
                              "1. Log in to Google Voice if needed\n"
                              "2. Come back here and click 'SEND MESSAGES'")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to start Chrome: {e}")
    ...
    def send_messages(self):
        """Run the Chrome-specific send script"""
        ...
        script = Path("send_texts_date_filter.py")
        if not script.exists():
            messagebox.showerror("Error", "send_texts_date_filter.py not found!")
            return
        ...
        try:
            self.status_var.set("📤 Sending messages via Chrome...")
            ...
            messagebox.showinfo("Started", 
                              ...
                              "Chrome will stay open when done.")
        except Exception as e:
    ...
    def open_report(self):
        ...
        try:
            subprocess.Popen([sys.executable, "generate_report.py", "config_chrome.json"],
                           creationflags=subprocess.CREATE_NEW_CONSOLE)
