...
class PTextingVivaldi:
    def __init__(self, root):
        self.root = root
        self.root.title("P-Texting - VIVALDI VERSION")
        self.root.geometry("800x900")
        
        # Config file
        self.config_file = "config_VIVALDI.json"
        ...
        # Header
        header = ttk.Label(main_frame, text="P-TEXTING - VIVALDI VERSION", 
                          font=('Arial', 16, 'bold'), foreground='purple')
        header.grid(row=0, column=0, columnspan=3, pady=10)
        
        info_label = ttk.Label(main_frame, text="✅ VIVALDI keeps you logged in - Power user features!", 
                              foreground='green')
        info_label.grid(row=1, column=0, columnspan=3, pady=5)
        
        row = 2
        
        # Note about VIVALDI profile - not needed for remote debugging
        note_label = ttk.Label(main_frame, text="ℹ️ VIVALDI profile auto-detected from running VIVALDI instance", 
        ...
        # Account Label (hardcoded for VIVALDI)
        ttk.Label(settings_frame, text="Account Label:").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.account_label_var = tk.StringVar(value="account5")
        ...
        ttk.Label(settings_frame, text="(Auto: VIVALDI = account5)", foreground='gray', font=('Arial', 8)).grid(row=1, column=2, sticky=tk.W, padx=5)
        ...
        ttk.Button(button_frame, text="🚀 Start VIVALDI Debug", command=self.start_VIVALDI_debug, width=20).grid(row=0, column=2, padx=5)
        ...
        # Status
        self.status_var = tk.StringVar(value="Ready - VIVALDI Version")
    ...
    def save_config(self):
        config = {
            "browser": "VIVALDI",
            "browser_profile_path": "C:\\temp\\VIVALDI_debug_profile",  # Auto from start_VIVALDI_debug.bat
            "browser_binary_path": "",
            ...
            "database_path": "progress_VIVALDI.db",
            "log_path": ".\\logs\\run_VIVALDI.log",
    ...
    def load_config(self):
        ...
        try:
            ...
            # Profile path auto-detected from running VIVALDI, no need to show/load
            ...
            self.account_label_var.set(config.get('account_label', 'account5'))
            
            self.status_var.set(f"✅ Config loaded from {self.config_file}")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to load config: {e}")
    
    def start_VIVALDI_debug(self):
        """Launch VIVALDI with remote debugging"""
        bat_file = Path("start_VIVALDI_debug.bat")
        if not bat_file.exists():
            messagebox.showerror("Error", "start_VIVALDI_debug.bat not found!")
            return
        ...
        try:
            ...
            self.status_var.set("🚀 VIVALDI debug mode started - Check the new window")
            messagebox.showinfo("VIVALDI Started", 
                              "VIVALDI has been started with remote debugging.\n\n"
                              "1. Log in to Google Voice if needed\n"
                              "2. Come back here and click 'SEND MESSAGES'")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to start VIVALDI: {e}")
    ...
    def send_messages(self):
        """Run the VIVALDI-specific send script"""
        ...
        script = Path("send_texts_date_filter_VIVALDI.py")
        if not script.exists():
            messagebox.showerror("Error", "send_texts_date_filter_VIVALDI.py not found!")
            return
        ...
        try:
            self.status_var.set("📤 Sending messages via VIVALDI...")
            ...
            messagebox.showinfo("Started", 
                              ...
                              "VIVALDI will stay open when done.")
        except Exception as e:
    ...
    def open_report(self):
        ...
        try:
            subprocess.Popen([sys.executable, "generate_report.py", "config_VIVALDI.json"],
                           creationflags=subprocess.CREATE_NEW_CONSOLE)
