#!/usr/bin/env python3
"""
P-TEXTING - BRAVE VERSION
Standalone Brave-only version with dedicated GUI

This is a completely separate program for Brave browser.
"""
...
class PTextingBrave:
    def __init__(self, root):
        self.root = root
        self.root.title("P-Texting - BRAVE VERSION")
        self.root.geometry("800x900")
        
        # Config file
        self.config_file = Path("configs/config_brave.json")
        ...
        # Header
        header = ttk.Label(main_frame, text="P-TEXTING - BRAVE VERSION", 
                          font=('Arial', 16, 'bold'), foreground='orange')
        header.grid(row=0, column=0, columnspan=3, pady=10)
        
        info_label = ttk.Label(main_frame, text="✅ Brave keeps you logged in - Privacy-focused!", 
                              foreground='green')
        info_label.grid(row=1, column=0, columnspan=3, pady=5)
        
        row = 2
        
        # Note about Brave profile - not needed for remote debugging
        note_label = ttk.Label(main_frame, text="ℹ️ Brave profile auto-detected from running Brave instance", 
        ...
        # Account Label (hardcoded for Brave)
        ttk.Label(settings_frame, text="Account Label:").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.account_label_var = tk.StringVar(value="account3")
        ...
        ttk.Label(settings_frame, text="(Auto: Brave = account3)", foreground='gray', font=('Arial', 8)).grid(row=1, column=2, sticky=tk.W, padx=5)
        ...
        ttk.Button(button_frame, text="🚀 Start Brave Debug", command=self.start_brave_debug, width=20).grid(row=0, column=2, padx=5)
        ...
        # Status
        self.status_var = tk.StringVar(value="Ready - Brave Version")
    ...
    def save_config(self):
        config = {
            "browser": "brave",
            "browser_profile_path": "C:\\temp\\brave_debug_profile",  # Auto from start_brave_debug.bat
            "browser_binary_path": "",
            ...
            "database_path": "progress_brave.db",
            "log_path": ".\\logs\\run_brave.log",
    ...
    def load_config(self):
        ...
        try:
            ...
            # Profile path auto-detected from running Brave, no need to show/load
            ...
            self.account_label_var.set(config.get('account_label', 'account3'))
            
            self.status_var.set(f"✅ Config loaded from {self.config_file}")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to load config: {e}")
    
    def start_brave_debug(self):
        """Launch Brave with remote debugging"""
        bat_file = Path("browser_launchers/start_brave_debug.bat")
        if not bat_file.exists():
            messagebox.showerror("Error", "start_brave_debug.bat not found!")
            return
        ...
        try:
            ...
            self.status_var.set("🚀 Brave debug mode started - Check the new window")
            messagebox.showinfo("Brave Started", 
                              "Brave has been started with remote debugging.\n\n"
                              "1. Log in to Google Voice if needed\n"
                              "2. Come back here and click 'SEND MESSAGES'")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to start Brave: {e}")
    ...
    def send_messages(self):
        """Run the Brave-specific send script"""
        ...
        script = Path("browser_specific/send_texts_date_filter_brave.py")
        if not script.exists():
            messagebox.showerror("Error", "send_texts_date_filter_brave.py not found!")
            return
        ...
        try:
            self.status_var.set("📤 Sending messages via Brave...")
            # Run in new console window using cmd /c start
            script_dir = Path.cwd()
            cmd = f'cmd /c start "Brave Sending" /wait cmd /c "{sys.executable}" "{script.absolute()}" ^& pause'
            ...
            messagebox.showinfo("Started", 
                              ...
                              "Brave will stay open when done.")
        except Exception as e:
    ...
    def open_report(self):
        ...
        try:
            subprocess.Popen([sys.executable, "generate_report.py", "configs/config_brave.json"],
                           creationflags=subprocess.CREATE_NEW_CONSOLE)
